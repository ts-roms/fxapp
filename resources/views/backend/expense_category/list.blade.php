					    ...
					    <tr data-id="row_{{ $expensecategory->id }}">
							...
							<td class="text-center">
								<span class="dropdown">
								  ...
								  <form action="{{ action('ExpenseCategoryController@destroy', $expensecategory['id']) }}" method="post">
									{{ csrf_field() }}
									<input name="_method" type="hidden" value="DELETE">

									<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
										<a href="{{ action('ExpenseCategoryController@edit', $expensecategory['id']) }}" data-title="{{ _lang('Update Expense Category') }}" class="dropdown-item dropdown-edit ajax-modal"><i class="ti-pencil-alt"></i>&nbsp;{{ _lang('Edit') }}</a>

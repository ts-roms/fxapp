					    ...
					    <tr data-id="row_{{ $memberdocument->id }}">
							...
							<td class="text-center">
								<span class="dropdown">
								  ...
								  <form action="{{ action('MemberDocumentController@destroy', $memberdocument['id']) }}" method="post">
									{{ csrf_field() }}
									<input name="_method" type="hidden" value="DELETE">

									<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
										<a href="{{ action('MemberDocumentController@edit', $memberdocument['id']) }}" data-title="{{ _lang('Update Document') }}" class="dropdown-item dropdown-edit ajax-modal"><i class="ti-pencil-alt"></i>&nbsp;{{ _lang('Edit') }}</a>

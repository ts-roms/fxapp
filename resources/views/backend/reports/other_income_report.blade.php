	...
	<div class="col-12">
		<div class="card">
			<div class="card-header">
				<span class="panel-title">{{ _lang('Other Income Report') }}</span>
			</div>

			<div class="card-body">

				<div class="report-params">
					<form class="validate" method="post" action="{{ route('reports.other_income_report') }}">
						<div class="row">
              				{{ csrf_field() }}
							...
							<div class="col-xl-3 col-lg-4">
								...
								<label class="control-label">{{ _lang('Other Income Category') }}</label>
									<select class="form-control auto-select" data-selected="{{ isset($category) ? $category : old('category') }}" name="category">
										...
										{{ create_option('other_income_categories','id','name',old('category')) }}
									</select>
								</div>
							</div>
				...
				<div class="report-header">
				   <h4>{{ _lang('Other Income Report') }}</h4>
				...
				<table class="table table-bordered report-table">
					<thead>
                        ...
                        <th>{{ _lang('Other Income Type') }}</th>
                        <th class="text-right">{{ _lang('Amount') }}</th>
					</thead>
					<tbody>
					@if(isset($report_data))
						@foreach($report_data as $other_income)
							<tr>
								<td>{{ $other_income->other_income_date }}</td>
								<td>{{ $other_income->reference }}</td>
								<td>{{ $other_income->other_income_category->name }}</td>
								<td class="text-right">{{ decimalPlace($other_income->amount, $currency) }}</td>
							</tr>
						@endforeach
						@if(count($report_data) > 0)
							<tr>
								<td>{{ $other_income->other_income_date }}</td>
								<td></td>
								<td><b>{{ _lang('Total Other Income') }}</b></td>

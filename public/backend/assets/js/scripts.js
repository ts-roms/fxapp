(function ($) {
	...
	if ($(".select2-ajax").length) {
		$('.select2-ajax').each(function (i, obj) {
			...
			$(this).select2({
				...
				target_select = $(this);
				$(".select2-results:not(:has(a))").append('<p class="border-top m-0 p-2"><a class="ajax-modal-2" href="' + $(this).data('href') + '" data-title="' + $(this).data('title') + '" data-reload="false"><i class="ti-plus"></i> ' + $lang_add_new + '</a></p>');
			});
	...
	$(document).on('change', '#client_login', function () {
		if ($(this).is(':checked') == false) {
			$("#client_login_card input, #client_login_card select").prop("disabled", true);
			$("#client_login_card input, #client_login_card select").prop("required", false);
		} else {
			$("#client_login_card input, #client_login_card select").prop("disabled", false);
			$("#client_login_card input, #client_login_card select").prop("required", true);
		}
	});
